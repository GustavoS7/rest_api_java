...
public class ProdutoController {
  ...
  public ResponseEntity<List<ProdutoResponse>> obterTodos() {
    ...
    List<ProdutoResponse> response = produtoDTOs.stream()
      .map(el -> mapper.map(el, ProdutoResponse.class))
      .collect(Collectors.toList());
    return new ResponseEntity<>(response, HttpStatus.OK);
  }

  @GetMapping("/{id}")
  public ResponseEntity<Optional<ProdutoResponse>> obterPorId(
    @PathVariable Integer id
  ) {
    Optional<ProdutoDTO> produtoDto = produtoService.obterPorId(id);
    ModelMapper mapper = new ModelMapper();
    ProdutoResponse response = mapper.map(
      produtoDto.get(),
  ...
  @PostMapping
  public ResponseEntity<ProdutoResponse> adicionar(
    @RequestBody ProdutoRequest produtoReq
  ) {
    ModelMapper mapper = new ModelMapper();
    ProdutoDTO dto = mapper.map(
      produtoReq,
      ProdutoDTO.class
    );
